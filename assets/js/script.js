...
function questionSwitch (previousQuestionElement, nextQuestionElement) {
    ...
    previousQuestionElement.animate({opacity: 0}, 500, `swing`, function () {
        ...
        // Now that the old question is gone, make the new question element the
        // current question by adding ID questionWrapper and class questionSpace
        nextQuestionElement.attr(`id`, `questionWrapper`);
        nextQuestionElement.addClass(`questionSpace`)
...
$(`#nextQuestion`).on(`click`, function () {
    counter++;
    let div = $(`<h1 class="text-center white">Hello! ` + counter + `</h1>`);
